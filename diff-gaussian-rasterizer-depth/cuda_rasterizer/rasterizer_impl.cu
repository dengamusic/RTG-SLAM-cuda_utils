...
CudaRasterizer::ImageState CudaRasterizer::ImageState::fromChunk(char *&chunk, size_t N)
{
	...
	obtain(chunk, img.hit_normal_c, N, 128);
	obtain(chunk, img.hit_point_c, N, 128);
	...
	bool debug)
{
	...
	// std::cout << "rendered count: " << num_rendered << std::endl;
	...
	CHECK_CUDA(cudaMemcpy(ranges_cpu, imgState.ranges, tile_grid.x * tile_grid.y * sizeof(uint2), cudaMemcpyDeviceToHost), debug);
	...
	for (int y = 0; y < tile_grid.y; y++)
	{
		...
		{
			const int tile_index = y * tile_grid.x + x;
			if (ranges_cpu[tile_index].x != ranges_cpu[tile_index].y)
			{
				tile_indices_cpu.emplace_back(tile_index);
			}
		}
	}
	...
	// printf("num rendered: %d, render_tile: %d\n", num_rendered, tile_num);
	// Let each tile blend its range of Gaussians independently in parallel
	const float *feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	// float3 campos{viewmatrix[12], viewmatrix[13], viewmatrix[14]};
	...
	// 			   imgState.hit_normal_c,
	// 			   imgState.hit_point_c,
	// 			   out_color,
	// 			   out_depth,
	// 			   out_hit_depth,
	// 			   out_hit_color,
	...
	// 			   out_T),
	// 		   debug)
	CHECK_CUDA(FORWARD::render_flat(
				   tile_indices,
				   tile_indices_cpu.size(),
				   tile_grid.x,
				   block,
				   focal_x, focal_y,
				   cx, cy,
	...
	bool debug)
{
	...
	CHECK_CUDA(BACKWARD::render_flat(
				   tile_indices,
				   tile_num,
				   tile_grid.x,
				   block,
